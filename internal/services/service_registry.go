...
type ServiceRegistryInterface interface {
	...
	StartServices() error
	StopServices() error
	RegisterServices(config *utils.Config, deviceInfo identity.DeviceInfoInterface) error
}
...
// StopServices stops all registered services in reverse order
func (sr *ServiceRegistry) StopServices() error {
	var err error
	for el := sr.services.Back(); el != nil; el = el.Prev() {
		name := el.Key
		svc := el.Value

		sr.Logger.Info().Msgf("Stopping service: %s", name)
		if stopErr := svc.Stop(); stopErr != nil {
			sr.Logger.Error().Err(stopErr).Msgf("Failed to stop service: %s", name)
			err = stopErr // Capture the error but continue stopping other services
		}
	}
	return err
}
	...
	for name, createService := range serviceConfigs {
		...
		case "registration":
			if config.Services.Registration.Enabled {
				...
				sr.Logger.Info().Msgf("%s service registered", name)
			}
		case "heartbeat":
